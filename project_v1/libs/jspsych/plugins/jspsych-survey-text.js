...
jsPsych.plugins['survey-text'] = (function() {
  ...
  plugin.trial = function(display_element, trial) {
    ...
    for (var i = 0; i < trial.questions.length; i++) {
      ...
      if(question.rows == 1){
        html += '<input type="text" id="input-'+question_index+'"  class=jspsych-survey-text name="#jspsych-survey-text-response-' + question_index + '" data-name="'+question.name+'" size="'+question.columns+'" '+autofocus+' '+req+' placeholder="'+question.placeholder+'"';
		if (question.defaultvalue) {
			html += ' value= "' +question.defaultvalue +'"';
		}
		html += '></input>';
      } else {
        html += '<textarea id="input-'+question_index+'" class=jspsych-survey-text name="#jspsych-survey-text-response-' + question_index + '" data-name="'+question.name+'" cols="' + question.columns + '" rows="' + question.rows + '" '+autofocus+' '+req+' placeholder="'+question.placeholder +'"';
		if (question.defaultvalue) {
			html += ' value= "' +question.defaultvalue +'"';
		}
		html += '></textarea>';
